...
def recent_conversation_messages() -> list[dict[str, str]]:
    messages: list[dict[str, str]] = []
    for round_item in read_communicate().get("rounds", []):
...
def delete_temp_rounds(ids: list[str]) -> dict[str, Any]:
    ...
    temp["rounds"] = [item for item in temp.get("rounds", []) if str(item.get("id", "")) not in id_set]
    return write_temp_communicate(temp)
...
def _temp_digest_due(state: dict[str, Any], stats: dict[str, Any], now: datetime) -> bool:
    ...
    if (stats["oldest_age_seconds"] or 0) < TEMP_DIGEST_MIN_AGE_SECONDS:
        return False
    last_digest_age = seconds_since(state.get("last_temp_digest_at"), now)
    return last_digest_age is None or last_digest_age >= TEMP_DIGEST_COOLDOWN_SECONDS
...
def prepare_heartbeat_state() -> dict[str, Any]:
    ...
    patch: dict[str, Any] = {
        ...
        "current_mode": "heartbeat",
        "last_heartbeat_at": current_time,
        "play": play_patch,
    }
    if digest_due:
        patch["last_temp_digest_at"] = current_time
